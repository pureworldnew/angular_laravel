...
var dateUtil = (function () {
  ...
  function getEndDate(startDateTime, startTime, timeType, durationHours, durationDays) {
    ...
    if (parseInt(timeType, 10) === 2 && durationDays != "") {
      ...
      endDateTime.setDate(endDateTime.getDate() + parseInt(durationDays));
    }
    /*else if (timeType == "2")
     {
...
new Vue({
  ...
  created: function () {
    ...
    function checkTodayHoliday(){
      var holiday_temp=document.getElementById("holiday").value;
        holiday_temp = holiday_temp.split(',');
        //console.log(holiday_temp);
        this.newarray = [];
        for(var i=0;i<holiday_temp.length;i++){
          var newstr = holiday_temp[i];
         var dateHoliday = new Date(newstr.replace(" ", ""));
          var string = jQuery.datepicker.formatDate('yy-mm-dd', dateHoliday);
          this.newarray.push(string);
        }
        return this.newarray;
    }
    function checkHoli(listOfHoli ,todayDate) {
       // console.log(listOfHoli)
        // console.log(todayDate)
      $.map(listOfHoli , function(item , i){
        var currentDate = new Date(todayDate)
        // console.log(item +'======'+todayDate)
        if(item == todayDate){
          currentDate.setDate(currentDate.getDate() + 1);
          window.finalDate = currentDate.toISOString().split('T')[0]
     
          checkHoli(listOfHoli , currentDate.toISOString().split('T')[0])

          // console.log(i + "-" +currentDate.toISOString().split('T')[0])
        }
      })
        window.finalDate = typeof(window.finalDate) == 'undefined' ? todayDate  : window.finalDate
      // console.log(window.finalDate)
      return window.finalDate
    }
    ...
    $("#startDate").datepicker({
      ...
      {
        var disdate = jQuery.datepicker.formatDate('yy-mm-dd', date);
        // window.holiDayArray = 
        var getArray = checkTodayHoliday();
        //console.log(getArray)
        // console.log(window.holiDayArray)
           return [getArray.indexOf(disdate)==-1];
      },
      dateFormat: "yy-mm-dd",
      firstDay: 1,
      defaultDate: window.finalDate,
    ...
    // }
    var listOfHoli = checkTodayHoliday();
    checkHoli(listOfHoli ,todayDate );
    var todayString = (today.getFullYear()) + '-' + ('0' + (parseInt(today.getMonth(), 10) + 1)).slice(-2) + '-' + ('0' + today.getDate()).slice(-2);
    $("#startDate").val(window.finalDate);
  
    that.startDate = window.finalDate;
  ...
  methods: {
    ...
    totalReservationCalc: function () {

      setTimeout(this.loadKlarnaPaymentWidget, 2000);

      this.totalReserve = this.shoppingCart.reduce(function (totalReservePrice, cartItem) {

        var reservePrice = parseInt(cartItem.reservepercentage*cartItem.price/100, 10)
        return totalReservePrice + reservePrice;
        //return total + (parseFloat(cartItem.price) * parseInt(cartItem.quantity));
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          var result = Object.assign({}, that.results[resultId], {
            ...
            updated_quantity: response.quantityAvailabile,
          });
          ...
          if (response.data.length > 0) {
            ...
            if (parseInt(response.quantityAvailabile, 10) > 0) {
              ...
              if (that.addToProductIdArray(result)) {
                isAvailable = true;
                that.results[resultId].updated_quantity = parseInt(response.quantityAvailabile - quantity, 10);
              }
              else {
    ...
    var startDateTime = new Date($('input[name="startDate"]').val());
    durDate = startDateTime.getDate();
    durMonth = startDateTime.getMonth() + 1;
    durYear = startDateTime.getFullYear();
    ...
    showSearchResult: function (responseData, key) {
      ...
      if (result.per_type_id == 1) {
        result = Object.assign({}, result, {
          ...
          durationDays: "",
          quantity: "",
          updated_quantity:result.availableProduct,
          hasPriceAvailCriteria: false
        });
      }
    ...
    removeCartItem: function (bookingLocationId, price,resultId) {
      //jpf
        ...
        $.ajax({
          ...
          success: function (success) {
            ...
            if (parseInt(res.status, 10) === 1) {
              ...
              that.getPrice(res.data.product_id, res.data.startDateTime, res.data.endDateTime, res.data.quantity, resultId, res.data.per_type_time_id);
    ...
    addToBooking: function (productId, resultId, productAvailable, availMessage) {
      if (productAvailable) {
        ...
        if (!this.bookingMade) {
          this.makeBooking(resultId);
          this.bookingMade = true;
        }
        else {
          this.addItemToBooking(this.bookingId, that.shoppingCart[that.shoppingCart.length - 1], that.shoppingCart.length - 1,resultId);
        }
        ...
        $('.modal-body').text(this.availabilityMessage)
      ...
      this.shoppingCart.push(
        {
          ...
          persons: resultItem.persons,
          reservepercentage: resultItem.reservepercentage
        });
    ...
    addToCart: function (resultId, result) {
      ...
      if (parseInt(result.quantity, 10) > 0) {
        ...
        function calcOffsetTime(startingTime, duration, offsetStart, offsetEnd,holiday_dates) {
          let day=new Date(holiday_dates);
          var holiday_dates_temp = $('#holiday').val();
          holiday_dates_temp = holiday_dates_temp.split(',');
          
          
          let startTime = new Date(startingTime);
          console.log ("FIRST START DATE", startTime)
          startTime.setHours(startTime.getHours() + duration);
          let offsetStartTime = new Date('2019-05-02 ' + offsetStart);
          let offsetEndTime = new Date('2019-05-02 ' + offsetEnd);
          let closingHour = offsetEndTime.getHours();
          let openingHour = offsetStartTime.getHours();
          let endingHour = startTime.getHours();
          if (openingHour > endingHour) {
            startTime.setHours(openingHour);
          }
          if (endingHour > closingHour) {
            startTime.setHours(openingHour);
            startTime.setDate(startTime.getDate() + 1);
                 
          }

          for(var i=0;i<holiday_dates_temp.length;i++){
            var date = new Date(startTime); // M-D-YYYY
            var d = date.getDate();
            var m = date.getMonth() + 1;
            var y = date.getFullYear();
            var dateString = (d <= 9 ? '0' + d : d) + '-' + (m <= 9 ? '0' + m : m) + '-' + y;
           
              var date1 = new Date(holiday_dates_temp[i]); // M-D-YYYY
              var d1 = date1.getDate();
              var m1 = date1.getMonth() + 1;
              var y1 = date1.getFullYear();
              var holidayString = (d1 <= 9 ? '0' + d1 : d1) + '-' + (m1 <= 9 ? '0' + m1 : m1) + '-' + y1;
              console.log ("HOLIDAY DATE", holidayString)
        ...
        var startTimeOffset = document.querySelector('#startTime').value;
        var endTimeOffset = document.querySelector('#endTime').value;
        result.endDateTime=dateUtil.formatDate(calcOffsetTime(new Date(endDateString), 0, startTimeOffset, endTimeOffset));
    ...
    sendBooking: function (booking,resultId) {
      var that = this;
      ...
      $.ajax({
        ...
        success: function (bookingDetails) {
          ...
          var cartItem = Object.assign({}, that.shoppingCart[0], { bookingLocationId: bookSplit[1], price: bookSplit[2], resultId: resultId })
    ...
    addItemToBooking: function (bookingId, shoppingCartItem, cartIndex,resultId) {
...
*/
    //makeBooking: function (event,resultId) {
    makeBooking: function (resultId) {
      ...
      this.sendBooking(booking,resultId);
